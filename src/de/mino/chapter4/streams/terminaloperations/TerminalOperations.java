...
/**
 * Shows the TERMINAL OPERATION part of streams.
 */
public class TerminalOperations {
     ...
     * }</pre>
     * <p>
     * Pass a custom comparator. Returns an Optional to specify that no min/max value was found.
     ...
     * }</pre>
     * <p>
     * Returns an empty Optional if the Stream was empty. Not a reduction
     * (because it doesn't necessarily look at all elements of the stream)
     * Works for infinite streams.
     * findAny() makes especially sense for parallel streams to just get a representative element.
     */
     ...
     * }</pre>
     * <p>
     * May or may not terminate on infinite streams. Not a reduction.
     */
     ...
     * }</pre>
     * <p>
     * Does not terminate on infinite streams. Not a reduction.
     */
     ...
     * Is a reduction.
     *
     * <ul><li> When an identity is defined: Start with the initial value (identity) and keep merging it with the next
     * element/ value of the stream. The accumulator combines the current value (starting with the identity) with the
     * elements in the stream. </li>
     * <li> When no identity is defined: Optional is returned because there might not be any data.
     * Stream empty --> Return an empty optional
     * Stream has 1 elem --> Return that element
     * Stream has multiple elements --> Use the accumulator to combine them. </li>
     * <li> The third method signature is used when processing collections in parallel. It allows us to create
     * intermediate reductions and combine them at the end.</li></ul>
     */
     ...
     * }</pre>
     * <p>
     * Mutable reduction. It is more efficient than a regular reduction because we use the same mutable object while
     * accumulating.
     * <ul>
     *  <li> First param: Supplier that creates the object that will  store the results as we collect data. </li>
     *  <li> Second parameter: BiConsumer, adds one more element to the data collection. </li>
     *  <li>Third parameter: BiConsumer which is responsible for taking two data collections and merges them. Useful when
     * processing in parallel.</li></ul>
     */
    private static void collect() {
        System.out.println("Terminal operation: collect()");
        List<String> list = Arrays.asList("w", "o", "l", "f");
        StringBuilder word = list.stream().collect(StringBuilder::new, StringBuilder::append, StringBuilder::append);
        ...
        System.out.println(set3); // [f, w, l, o] (output might be different, we are just likely to get a HashSet)
        System.out.println();
    }
